...
func newTestMCPServer(t *testing.T) *mcp.Server {
	...
	toolset := toolsets.NewToolset("test", "toolset used by the config tests")
	toolset.AddReadTools(
		newTestReadTool("twprojects-read"),
		newTestReadTool("twdesk-read"),
	)
	toolset.AddPrompts(toolsets.NewServerPrompt(
		&mcp.Prompt{Name: "twprojects-test_prompt"},
		func(context.Context, *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
			return &mcp.GetPromptResult{}, nil
		},
	))
	toolset.AddResources(toolsets.NewServerResource(
	...
	return NewMCPServer(resources, group)
}
