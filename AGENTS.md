...
- Moving a task between tasklists carries its subtasks and detaches an invalidated parent, both server-side. `PATCH /projects/api/v3/tasks/{id}.json` (the SDK's `TaskUpdate`, behind `twprojects-update_task`) routes into `TasksService.Edit`, which calls `detachParentOnMove` and then `Move`; `Move` recursively moves the subtasks. `detachParentOnMove` only drops a parent the payload *inherited* from the task being edited — a parent the caller sent explicitly is still validated, so passing `parent_task_id` alongside a `tasklist_id` change still fails with "parent task is in another list". Do not send one when moving. This landed in projectsapigo `932f6ae200` (2026-08-10); before it, v3 moved only the named task and left the subtree stranded, which is why the client had to move each descendant by hand. `twprojects-move_tasks` is now a plain loop of task updates and exists only to batch several subtrees into one tool call.
...
- `get_*` tools whose SDK request has a `Fields <Entity>GetFields` slot expose the same `fields` parameter — see `TaskGet` in `internal/twprojects/tasks.go`:
  - The slot lives on the request root (`req.Fields.<Entity>`), not under `Filters`, so it is `helpers.OptionalFieldsParam[projects.<Entity>](&req.Fields.<Entity>, "fields")`.
