...
- Moving a task between tasklists cascades on v1 but not on v3. `PATCH /projects/api/v3/tasks/{id}.json` (the SDK's `TaskUpdate`, behind `twprojects-update_task`) moves only the task it names, stranding its subtasks in a state that violates the API's own rule that a subtask shares its parent's tasklist — the caller then has to move every descendant by hand, parents first. `PUT /tasks/{id}.json` (v1, at the installation root, body `{"todo-item":{"id":…,"taskListId":…}}`) moves the whole subtree in one call and answers with `affectedTaskIds`, a comma-separated list of every task it touched. `twprojects-move_tasks` is built on the v1 endpoint for exactly this reason; the SDK does not model it, so `taskMoveV1Request` in `internal/twprojects/tasks.go` builds it. Neither endpoint is documented to this level — verify against a live installation before assuming a v3 write cascades.
