...
// SetNamespace records the prefix every tool name in this group shares
// ("twprojects") and the OAuth scope that grants access to them ("projects").
//
// The server uses both to filter tools/list down to what the caller's token
// actually allows. A group that sets no namespace is never filtered, so a
// server exposing scoped tools must set one — otherwise the tools are listed to
// every token regardless of scope.
func (tg *ToolsetGroup) SetNamespace(toolPrefix, scope string) *ToolsetGroup {
	tg.toolPrefix = toolPrefix
	tg.scope = scope
	return tg
}

// ToolPrefix returns the prefix every tool name in this group shares, or an
// empty string when the group declared none.
func (tg *ToolsetGroup) ToolPrefix() string {
	return tg.toolPrefix
}

// Scope returns the OAuth scope that grants access to this group's tools, or an
// empty string when the group declared none.
func (tg *ToolsetGroup) Scope() string {
	return tg.scope
}

// Scopes returns the distinct OAuth scopes the given groups declare, sorted.
// This is what a server advertises as "scopes_supported" in its RFC 9728
// protected-resource metadata.
func Scopes(groups []*ToolsetGroup) []string {
	...
	for _, group := range groups {
		...
		if _, ok := seen[group.scope]; ok {
			continue
		}
		seen[group.scope] = struct{}{}
